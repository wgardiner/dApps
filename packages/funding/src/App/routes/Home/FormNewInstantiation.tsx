			...
			>
				<Formik
					...
					onSubmit={async (values, { setSubmitting, setFieldError }) => {
						...
						const params = {
							...
							proposerWhitelist: values.proposerWhitelist
								.split(',')
								.map((s) => s.trim()),
							voterWhitelist: values.voterWhitelist
								.split(',')
								.map((s) => s.trim()),
              ...
              <FormItem name="tags" label="Project tags">
                <Input name="tags" />
							...
							<FormItem
								label="Proposer whitelist (comma separated addresseses)"
								name="proposerWhitelist"
							>
								<Input name="proposerWhitelist" />
							</FormItem>
							<Form.Item
								label="Voter whitelist (comma separated addresses)"
								name="voterWhitelist"
							>
							...
							<Form.Item label="Funding Pool (in ushell)" name="budgetAmount">
								<WideInputNumber
									style={{ width: '100%' }}
									name="budgetAmount"
									// formatter={(value) => `${value} ushell`}
									// parser={(value) => value.replace('ushell', '')}
								/>
							</Form.Item>
							<Input name="budgetDenom" type="hidden" value="ushell" />
